...
// Для "разгона" режима 2: по одному светодиоду каждые 0.5 с
...
int main(void)
{
    ...
    while (1)
    {
        /* Обработка кнопок (флаги поставлены в прерываниях) */
...
static void Button1_Task(void)
{
    ...
    current_mode ^= 1U; // XOR с 1: 0→1, 1→0
    ...
    else
    {
        ...
        for (uint8_t i = 0; i < LED_COUNT; i++)
        {
            led_state[i] = 0U;
            last_toggle_ms[i] = system_time_ms;
            /* freq2_index[i] можно НЕ трогать, чтобы частоты сохранялись */
        }

        selected_led = 0U;

        /* Сбрасываем "фазы" режима 2 */
 ...
 *  - в режиме 1: можно игнорировать (по заданию нужна в режиме настройки мерцания)
 */
static void Button3_Task(void)
{
...
static void Mode1_Task(void)
{
    ...
    {
    case 0U:
        /* 3-й и 4-й светодиоды → индексы 2 и 3 */
        LED_On_Index(2U);
        LED_On_Index(3U);
        break;

    case 1U:
        /* 2-й и 5-й светодиоды → индексы 1 и 4 */
        LED_On_Index(1U);
        LED_On_Index(4U);
        break;
    ...
    case 2U:
    default:
        /* 1-й и 6-й светодиоды → индексы 0 и 5 */
        LED_On_Index(0U);
        LED_On_Index(5U);
        break;
    }
...
/* Режим 2:
 * Каждый светодиод мигает (мерцает) со своей частотой.
...
static void Mode2_Task(void)
{
    ...
    if (mode2_stage == 0U)
    {
        if (mode2_enabled_count < LED_COUNT)
        {
            if ((now - mode2_last_enable_ms) >= MODE2_ENABLE_STEP_MS)
            {
                ...
                // Если только что включили последний (6-й), фиксируем момент
                if (mode2_enabled_count >= LED_COUNT)
                {
    ...
    if (mode2_stage == 1U)
    {
        /* На всякий случай убедимся, что все включены */
        for (uint8_t i = 0; i < LED_COUNT; i++)
        {
            if (led_state[i] == 0U)
            {
        ...
        if ((now - mode2_all_on_time_ms) >= MODE2_ALL_ON_HOLD_MS)
        {
            ...
            /* Обнулим "отсчёт" для мерцания, чтобы все начали синхронно
               (каждый всё равно будет мигать со своей частотой) */
            for (uint8_t i = 0; i < LED_COUNT; i++)
            {
